...
from odl.core.operator import Operator, PointwiseInner, OperatorPointwiseProduct
...
from odl.core.array_API_support import exp, lookup_array_backend, get_array_and_backend

__all__ = ('NonLinDeformFixedTempl', 'NonLinDeformFixedDisp', 'nonlinear_deform', 'LDDMMFixedTempl', 'MassPreservDeformFixedTempl')

def nonlinear_deform(template, displacement, interp='linear', out=None):
    """Non-linear deformation of a template with a displacement field.

    The function maps a given template ``I`` and a given displacement
    field ``v`` to the new function ``x --> I(v(x))``.
...
class NonLinDeformFixedTempl(Operator):
    ...
    See Also
    --------
    NonLinDeformFixedDisp : Deformation with a fixed displacement.
    ...
    .. math::
        ...
        \big< \nabla I(v(\cdot)), u \big>_{\mathbb{R}^d},
    ...
    def __init__(self, template, domain=None, interp='linear'):
        ...
        super(NonLinDeformFixedTempl, self).__init__(
    ...
    def _call(self, displacement, out=None):
        ...
        return nonlinear_deform(self.template, displacement, self.interp, out)
    ...
    def derivative(self, displacement):
        ...
        def_grad = self.domain.element(
            [nonlinear_deform(gf, displacement, self.interp) for gf in grad_templ]
        )
...
class NonLinDeformFixedDisp(Operator):
    ...
    ``I`` to the new function ``x --> I(v(x))``.

    See Also
    --------
    NonLinDeformFixedTempl : Deformation with a fixed template.
    ...
    .. math::
        W_v : X \to X, \quad W_v(I) := I(v(\cdot)),

    i.e., :math:`W_v(I)(x) = I(v(x))`.
    ...
    .. math::
        W_v^*(I) \approx \exp(-\mathrm{div}\, v) \, I( v(\cdot)),

    i.e., :math:`W_v^*(I)(x) \approx \exp(-\mathrm{div}\,v(x))\, I(v(x))`.
    """
    ...
    def __init__(self, displacement, templ_space=None, interp='linear'):
        ...
        super(NonLinDeformFixedDisp, self).__init__(
    ...
    def _call(self, template, out=None):
        ...
        return nonlinear_deform(template, self.displacement, self.interp, out)
...
class JacobianDet(Operator):
    def __init__(self, space, method='forward'):
        ...
        self.space = space
        self.dim = space.ndim
        self.grad = Gradient(space, method=method)

        super().__init__(
            domain=space.tangent_bundle,
            range=space
        )

    def _call(self, phi):

        grads = [self.grad(phi[i]) for i in range(self.dim)]

        arrays = []
        backend = None

        for g in grads:
            arr, backend_ar = get_array_and_backend(g)
            backend = backend_ar.array_namespace
            arrays.append(backend.moveaxis(arr, 0, -1))

        J = backend.stack(arrays, axis=-2)
        return backend.abs(backend.linalg.det(J))

class MassPreservDeformFixedTempl(Operator):
    def __init__(self, template):
        self.op = OperatorPointwiseProduct(
            NonLinDeformFixedTempl(template),
            JacobianDet(template.space)
        )
...
class LDDMMFixedTempl(Operator):
    def __init__(self, template, action, integrator, Nt, domain):
        """
        """
        
        def compose(v, phi, t=None):
            the_space = v.space
            defor = NonLinDeformFixedDisp(phi)
        ...
        super().__init__(domain=self.operator.domain, range=self.operator.range)


    def _call(self, vt):
        return self.operator(vt)
